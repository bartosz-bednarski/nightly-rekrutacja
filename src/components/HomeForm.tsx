...
const HomeForm: Component = () => {
  return (
    <div class="flex flex-col bg-background-component shadow-withdraw-shadow border-2 border-border-component h-492px text-white mt-16 mr-10 mb-52 ml-14 p-4 w-332px rounded-lg">
      ...
      >
        <div class="flex flex-row gap-2 text-sm">
          <img src={bitcoin} class="w-6 h-6" />
          <span>Bitcoin</span>
          <span class="text-label-color">(BTC)</span>
        </div>
      ...
      <label for="withdraw-to" class="text-label-color text-xs mt-4 mb-2 ">
        Withdraw to
      </label>
      <div class="flex flex-row content-between bg-input-background border-2 border-input-border py-6px px-3 gap-4">
        ...
        <svg
          ...
          xmlns="http://www.w3.org/2000/svg"
        >
          <circle cx="12" cy="12" r="12" fill="#6067F9" fill-opacity="0.25" />
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
      ...
      <button
        name="withdraw"
        class="mt-22px px-4 py-6px text-sm bg-withdraw-button-background rounded "
      >
        Withdraw
      </button>
    </div>
